...
def build_pdf(itinerary, hop_kms, maps_link, ordered_cities, days, prefs, parsed_requests, is_car_mode=False, result=None):
    ...
    for day in itinerary:
        ...
        for city_stop in cities_data:
            ...
            if attractions:
                ...
                for idx, attr in enumerate(attractions[:6], 1):
                    ...
                    if lat and lon:
                        attr_url = f"https://www.google.com/maps/search/?api=1&query={lat},{lon}"
                        pdf.set_x(pdf.get_x() + 5)
                        pdf.add_link('View on Google Maps', attr_url, COLOR_PRIMARY)
        ...
        if hotels and day_in_city == 1 and not is_hub_trip:
            ...
            if current_date_obj:
                try:
                    checkout_date = current_date_obj + timedelta(days=total_days_in_city)
            ...
            for hotel in hotels[:3]:
                ...
                pdf.set_x(pdf.get_x() + 5)
                pdf.add_link('Book on Booking.com', booking_url, COLOR_HOTEL)
                pdf.ln(1)
